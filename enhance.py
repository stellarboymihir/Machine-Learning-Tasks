...
class FingerprintImageEnhancer(object):
    ...
    def save_enhanced_image(self, path):
        ...
        cv2.imwrite(f"{path2}/{filename.split('.')[0]}.jpg", (255 * self._binim))
...
path1 = r"/home/mihir/Documents/Ml_Project/Database/Bagvan_Zaid1/"
path2 = r"/home/mihir/Documents/Ml_Project/Enhancement/Bagvan_Zaid1/"
        ...
        try:
            if filename.endswith(".jpg"):
                ...
                if (len(sys.argv) < 2):  # load input image
                    img_name = f"{path2}/{filename.split('.')[0]}.jpg"
                    img = cv2.imread(f"{path1}{filename}", 0)
                ...
                image_enhancer.save_enhanced_image(f"{path2}" + img_name)  # save output

                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=RuntimeWarning)
                    mean2 = np.nanmean([np.NaN, np.NaN])
                    print(mean2)
